...
# 預設整段 prompt（system + 指令）。使用者可於設定頁覆寫；逐字稿接在其後送出。
DEFAULT_SUMMARY_PROMPT = f"{SUMMARY_SYSTEM}\n\n{SUMMARY_INSTRUCTION}"


def build_summary_prompt(transcript: str, prompt: str | None = None) -> str:
    """把（自訂或預設）prompt 與逐字稿併成單一字串（走 stdin 餵 claude -p）。

    prompt 留空 → 用 DEFAULT_SUMMARY_PROMPT；自訂 prompt 會 strip 後補空行再接逐字稿。
    """
    prefix = (prompt or "").strip() or DEFAULT_SUMMARY_PROMPT
